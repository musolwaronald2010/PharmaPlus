...
create table if not exists public.products (
  ...
  expiry date not null, qty integer not null default 0 check(qty>=0), reorder integer not null default 10 check(reorder>=0),
...
declare pid uuid; assigned_role public.user_role := 'staff'; inv record; invite_code text := upper(trim(coalesce(new.raw_user_meta_data->>'invite_code','')));
begin
  if invite_code <> '' then
    select * into inv from public.staff_invites where code=invite_code and used_at is null and expires_at>now() limit 1 for update;
    if not found then raise exception 'Invalid or expired staff invitation code'; end if;
    pid:=inv.pharmacy_id; assigned_role:=inv.role;
    insert into public.profiles(id,email,full_name,role,pharmacy_id) values(new.id,new.email,coalesce(new.raw_user_meta_data->>'full_name',''),assigned_role,pid) on conflict(id) do update set pharmacy_id=excluded.pharmacy_id,role=excluded.role;
...
-- Repair accounts that existed before the trigger/schema was installed.
do $$ declare u record; pid uuid; begin
  for u in
    ...
    where p.id is null
  loop
    insert into public.pharmacies(name,location,contact,email,created_by)
    values(
      coalesce(nullif(trim(u.raw_user_meta_data->>'pharmacy_name'),''),'PharmaPlus Pharmacy'),
      nullif(trim(u.raw_user_meta_data->>'pharmacy_location'),''),
      nullif(trim(u.raw_user_meta_data->>'pharmacy_contact'),''),
      u.email,u.id
    ) returning id into pid;
    insert into public.profiles(id,email,full_name,role,active,pharmacy_id)
    values(u.id,u.email,coalesce(u.raw_user_meta_data->>'full_name',''),'admin',true,pid)
...
-- Staff invitation RLS policies must be created AFTER current_pharmacy() and has_role().
...
create policy staff_invites_insert on public.staff_invites for insert to authenticated with check(pharmacy_id=public.current_pharmacy() and public.has_role(array['admin']::public.user_role[]) and created_by=auth.uid());
